...
{/*
    ...
    await axios({
        method:'get',
        //headers: {Authorization:"bearer "+await AsyncStorage.getItem('tokenCookie')},
        url:'http://localhost:4000/suppliers/getAllSuppliers'
    }).then(async (response)=>{
await axios({
        method:'get',
        //headers: {Authorization:"bearer "+await AsyncStorage.getItem('tokenCookie')},
        url:'http://localhost:4000/deletedsuppliers/seeDeletions'
    }).then((resp)=>{
        var eliminations=[]
        resp.data.map((e)=>{eliminations.push(e.companyname)});
...
<>
{Object.keys(suppdata).length?<StockDialog  categories={categories} open={dialogOpen} cb={setDialogOpen} supp={suppdata}/>:null}
{Object.keys(suppdata).length?<StockUpdate categories={categories} open={updateBoolean} cb={setUpdateBoolean} update={toUpdate} supp={suppdata}/>:null}
</>
      </Box>
