...
import { AutoDelete,East,ArrowBack, DateRange, EventAvailable, Inventory, LocalShipping, QuestionMark, Restore, Update, UpdateDisabled, Person, Task, Mail, Highlight, Pin, Phone, Scale, Biotech, Sell, ProductionQuantityLimits, Kitchen, ContactEmergency, EventBusy, Add } from '@mui/icons-material';
...
import StockDialog from './AddDialog';
import StockUpdate from './UpdateDialog';
        ...
        <AppBar position="absolute" open={open}>
          ...
          >
            ...
            <IconButton
              ...
              color="inherit"
              aria-label="open drawer"
              onClick={()=>{if( data.length){data.map((e)=>{col.push({productName:e.productName})}), props.setQuery(col),navigation.navigate("Accountant")}else alert("no deletions to wipe")}}
            >
              <DeleteForeverIcon/>
              {/* <img  src={require("./ship-light.png")} height={23} width={20}/> */}
            </IconButton>
...
<GridCard  activeIngredient={e.activeIngredient}
prescOnly={e.prescOnly}refrigerate={e.refrigerate}
qty={e.qty}type={e.type}productName={e.productName} providers={e.providers}
 expires={e.expires} productImage={e.productImage} hl={e} cb={setHighlight}/>
                ...
                <NewGrid data={data} callbackUpdate={setUpdateBoolean} updateHook={setToUpdate}// activeIngredient={e.activeIngredient}
                // prescOnly={e.prescOnly}refrigerate={e.refrigerate}
                // qty={e.qty}type={e.type}productName={e.productName} providers={e.providers}
                //  expires={e.expires} productImage={e.productImage} hl={e} cb={setHighlight}
                />:null}
...
<>
{Object.keys(suppdata).length?<StockDialog open={dialogOpen} cb={setDialogOpen} supp={suppdata}/>:null}
{Object.keys(suppdata).length?<StockUpdate open={updateBoolean} cb={setUpdateBoolean} update={toUpdate} supp={suppdata}/>:null}
</>
      </Box>
