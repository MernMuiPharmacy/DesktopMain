...
{/*
  const onloadStaff=async ()=>{
    await axios({
        method:'get',
        //headers: {Authorization:"bearer "+await AsyncStorage.getItem('tokenCookie')},
        url:'http://localhost:4000/stock/getAllStock'
    }).then(async (response)=>{
      await axios({
        method:'get',
        //headers: {Authorization:"bearer "+await AsyncStorage.getItem('tokenCookie')},
        url:'http://localhost:4000/deletedstock/seeDeletions'
    }).then((resp)=>{
        var eliminations=[]
        resp.data.map((e)=>{eliminations.push(e.productName)});
        ...
        var toBe=[]
        response.data.map((e)=>{if(!eliminations.includes(e.productName)){toBe.push({qty:e.qty,prescOnly:e.prescOnly,price:e.price,type:e.type,activeIngredient:e.activeIngredient,providers:e.providers,productName:e.productName,productImage:e.productImage,expires:e.expires,refrigerate:e.refrigerate})}});
        console.log(toBe) 
       setData(toBe)
    console.log(data)
...
{Object.keys(data).length?<NewGrid data={data} callbackUpdate={setIsUpdating} updateHook={setToUpdate} setIsDeleting={setIsDeleting} setToDelete={setToDelete}/>:<CircularProgress/>}
       ...
       <DeleteDialog open={isDeleting} setOpen={setIsDeleting} respectiveItems={prodData.length?prodData:[]} toDelete={toDelete}/>
      </Box>
